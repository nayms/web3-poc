...
pub mod nayms {
    ...
    pub fn register_emitter(
        ...
        require!(
            ...
            NaymsError::InvalidForeignEmitter,
        );
    ...
    /// [NaymsMessage::Hello], which serializes a payload ID (1) before the message
    ...
    ///
    /// See [NaymsMessage] enum for serialization implementation.
    ///
    /// # Arguments
    ///
    ...
    pub fn send_message(ctx: Context<SendMessage>, message: Vec<u8>) -> Result<()> {
        ...
        let payload: Vec<u8> = NaymsMessage::Hello { message }.try_to_vec()?;
    ...
    /// that the payload is of type [NaymsMessage::Hello] (payload ID == 1). NaymsMessage
    /// data is stored in a [Received] account.
    ///
    /// See [NaymsMessage] enum for deserialization implementation.
    ///
    /// # Arguments
    ///
    ...
    pub fn receive_message(ctx: Context<ReceiveMessage>, vaa_hash: [u8; 32]) -> Result<()> {
        ...
        if let NaymsMessage::Hello { message } = posted_message.data() {
            // NaymsMessage cannot be larger than the maximum size of the account.
            require!(
                message.len() <= MESSAGE_MAX_LENGTH,
                NaymsError::InvalidMessage,
            );
            ...
            Err(NaymsError::InvalidMessage.into())
        }
    }
}
