...
pub struct RegisterEmitter<'info> {
    ...
    #[account(
        has_one = owner @ NaymsError::OwnerOnly,
        seeds = [Config::SEED_PREFIX],
        bump
    )]
...
pub struct SendMessage<'info> {
    ...
    #[account(
        mut,
        address = config.wormhole.bridge @ NaymsError::InvalidWormholeConfig
    )]
    ...
    #[account(
        mut,
        address = config.wormhole.fee_collector @ NaymsError::InvalidWormholeFeeCollector
    )]
    ...
    #[account(
        mut,
        address = config.wormhole.sequence @ NaymsError::InvalidWormholeSequence
    )]
...
type NaymsVaa = wormhole::PostedVaa<NaymsMessage>;
...
pub struct ReceiveMessage<'info> {
    ...
    pub posted: Account<'info, NaymsVaa>,
    ...
    #[account(
        ...
        bump,
        constraint = foreign_emitter.verify(posted.emitter_address()) @ NaymsError::InvalidForeignEmitter
    )]
