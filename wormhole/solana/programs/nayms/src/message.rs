...
pub enum NaymsMessage {
    Alive { program_id: Pubkey },
    Hello { message: Vec<u8> },
}

impl AnchorSerialize for NaymsMessage {
    fn serialize<W: io::Write>(&self, writer: &mut W) -> io::Result<()> {
        match self {
            NaymsMessage::Alive { program_id } => {
                PAYLOAD_ID_ALIVE.serialize(writer)?;
                program_id.serialize(writer)
            }
            NaymsMessage::Hello { message } => {
...
impl AnchorDeserialize for NaymsMessage {
    fn deserialize_reader<R: io::Read>(reader: &mut R) -> io::Result<Self> {
        match u8::read(reader)? {
            PAYLOAD_ID_ALIVE => Ok(NaymsMessage::Alive {
            ...
            PAYLOAD_ID_HELLO => {
                ...
                if length > WH_MESSAGE_MAX_LENGTH {
                    ...
                    Ok(NaymsMessage::Hello { message: buf })
                }
            }
            _ => Err(io::Error::new(
                io::ErrorKind::InvalidInput,
...
mod test {
    ...
    fn test_message_alive() -> Result<()> {
        ...
        match NaymsMessage::deserialize(&mut encoded.as_slice())? {
            NaymsMessage::Alive { program_id } => {
